...
api_key = st.text_input("Paste Gemini API key", type="password", help="Get a free key from https://aistudio.google.com/app/apikey")
model_option = st.selectbox(
    "Select Gemini Model",
    ["gemini-2.0-flash-exp", "gemini-2.5-flash", "gemini-1.5-flash", "gemini-1.5-flash-8b", "gemini-1.5-pro", "gemini-exp-1206"],
    index=0
)
...
try:
    ...
    response = client.models.generate_message(
        model=model_option,
        messages=[types.Message(role="user", content=[pdf_part, prompt_part])]
    )

    # Access text correctly
    if response and response.last and response.last.content:
        text = response.last.content[0].text.strip()
    else:
        text = ""
    ...
    if not text:
        ...
        try:
            ...
            st.json(parsed_json)
            st.download_button("💾 Download JSON", data=json.dumps(parsed_json, indent=2), file_name="extracted_data.json", mime="application/json")
